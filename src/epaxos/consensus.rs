...
use super::{
    ...
    replica::Replica,
    types::{Ballot, CommandLeaderID, LeaderBook, ReplicaID},
...
use crate::epaxos::{
    ...
    types::{InstanceID, InstanceStatus},
    util::instance_exist,
};
...
impl ConsensusImpl {
    pub async fn new(conf: Configure) -> Self {
        ...
        Self {
            ...
            replica: Arc::new(Mutex::new(Replica::new(id, peer_cnt))),
        }
    }

    pub fn conf(&self) -> &Configure {
        &self.conf
    }

    pub async fn new_leaderbook(&self) -> LeaderBook {
        LeaderBook::new(self.replica.lock().await.id, &self.conf)
    }

    pub async fn new_ballot(&self) -> Ballot {
    ...
    async fn broadcast_message(&self, replica: ReplicaID, message: Message) {
    ...
    async fn reply(&self, replica: ReplicaID, command_leader: &CommandLeaderID, message: Message) {
    ...
    async fn handle_propose(&self, propose: Propose) {
        ...
        let new_instance = SharedInstance::new(
            Some(Instance {
                ...
                leaderbook: self.new_leaderbook().await,
            }),
            None,
        );
    ...
    async fn handle_preaccept(&self, preaccept: PreAccept) {
    ...
    async fn handle_preaccept_reply(&self, preacceptreply: PreAcceptReply) {
        ...
        if !preacceptreply.ok {
            ...
            if preacceptreply.ballot > instance_w_inner.leaderbook.max_ballot {
                instance_w_inner.leaderbook.max_ballot = preacceptreply.ballot;
            }
            return;
        }
    ...
    async fn handle_accept_reply(&self, accept_reply: AcceptReply) {
        ...
        if !accept_reply.ok {
            ...
            if accept_reply.ballot > instance_write_inner.leaderbook.max_ballot {
                instance_write_inner.leaderbook.max_ballot = accept_reply.ballot;
            }
            return;
        }
