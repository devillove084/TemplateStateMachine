...
//         println!("Wait..");
//         let set_args = || consensus::SetArgs {
//             key: key.clone(),
//             value: value.clone(),
//         };
...
//     for _ in 0..(args.cmd_count.wrapping_div(args.batch_size)) {
//         let futures =
//             (0..args.batch_size).map(|_| server.get(consensus::GetArgs { key: key.clone() }));
//         let results = join_all(futures).await;
//         for result in results {
//             result?;
//         }
...
//     config: &Config,
// ) -> Result<BTreeMap<String, consensus::GetMetricsOutput>> {
//     let rpc_client_config = crate::default_rpc_client_config();
...
//     before: &BTreeMap<String, consensus::GetMetricsOutput>,
//     after: &BTreeMap<String, consensus::GetMetricsOutput>,
...
//             rhs.replica_recover_success_count - lhs.replica_recover_success_count;
//         executed_single_cmd_count += rhs.executed_single_cmd_count - lhs.executed_single_cmd_count;
//         executed_batched_cmd_count +=
//             rhs.executed_batched_cmd_count - lhs.executed_batched_cmd_count;
//     }
...
//     let mut generate = &RandomCmds::new(args.value_size, args.conflict_rate);
...
//                 let consensus::SetArgs { key, value } = generate.next().unwrap();
//                 clone!(server);
...
//             spawn(async move {
//                 let t0 = Instant::now();
//                 server.set(consensus::SetArgs { key, value }).await.unwrap();
//                 let t1 = Instant::now();
...
//                 // let metrics = server.get_metrics(consensus::GetMetricsArgs {}).await.unwrap();
...
//         let generate = RandomCmds::new(args.value_size, args.conflict_rate);
//         let working = wg.working();
//         clone!(stopped);
//         spawn(async move {
//             let mut interval = interval;
//             let mut cmds = generate.take(count).collect::<Vec<_>>();
//             loop {
//                 interval.tick().await;
...
//                 cmds = generate.take(count).collect::<Vec<_>>();
//             }
//             drop(working);
//         });
//     }
...
//     type Item = consensus::SetArgs;
//     #[allow(clippy::arithmetic_side_effects)]
//     fn next(&mut self) -> Option<consensus::SetArgs> {
...
//         Some(consensus::SetArgs { key, value })
//     }
// }
