use super::bounds::StatusBounds;
use super::deps::Deps;
use super::id::InstanceId;
use super::id::ReplicaId;
use super::id::Seq;
use super::status::ExecStatus;
use super::status::Status;

use crate::utils::cmp::min_assign;
use crate::utils::lock::with_mutex;
use crate::utils::watermark::WaterMark;
    ...
    pub fn init_node(&self, id: InstanceId, cmd: C, seq: Seq, deps: Deps, status: Status) {
        ...
        let generate = || {
            Asc::new(InsNode {
                ...
                seq,
                deps,
                status: SyncMutex::new(exec_status),
            })
        };
        match self.nodes.entry(id) {
            dashmap::mapref::entry::Entry::Occupied(mut e) => match e.get_mut() {
                ...
                Node::Waiting(n) => {
                    n.notify_waiters();
                    e.insert(Node::InGraph(generate()));
                }
            },
...
mod tests {
    ...
    fn build_graph(edges: &[(u64, u64)]) -> LocalGraph<u64, TestNode> {
        ...
        for &(u, v) in edges {
            let generate = || TestNode {
                deps: VecSet::new(),
            };

            nodes.entry(v).or_insert_with(generate);
            let u = nodes.entry(u).or_insert_with(generate);
    ...
    fn linear_and_cycle() {
        ...
        assert_scc(&ans, &[&[701, 702, 703, 801, 802, 803], &[704, 804], &[
            805,
        ]]);
    }
