...
use consensus::replica::{Replica, ReplicaMeta};
use protocol::{cs, rpc, sm};

use with_rocksdb::cmd::{BatchedCommand, Command, CommandKind, CommandNotify, Del, Get, MutableCommand, Set};
use with_rocksdb::data_db::DataDb;
use with_rocksdb::log_db::LogDb;

use utils::atomic_flag::AtomicFlag;
use utils::chan;
...
type ConsensusReplica = Replica<BatchedCommand, LogDb, DataDb, TcpNetwork<BatchedCommand>>;

pub struct Server {
    replica: Arc<ConsensusReplica>,
...
impl Server {
    pub async fn run(config: Config) -> Result<()> {
        let replica = {
            ...
            ConsensusReplica::new(meta, log_store, data_store, net).await?
        };
...
mod tests {
    ...
    use consensus::msg::Message;
    use utils::chan;
    use utils::func::output_size;

    use std::mem;

    #[test]
    fn replica_future_size() {
        assert_eq!(output_size(&ConsensusReplica::handle_message), 408);
        assert_eq!(output_size(&ConsensusReplica::run_propose), 416);
    }
